        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <Field label="UR/USA">
                <RadioGroup value={dadosOperacionais.ur_usa || ''} onValueChange={(v) => handleOperationalDataChange('ur_usa', v)} className="flex flex-row space-y-0 gap-4">
                    <div className="flex items-center space-x-2"><RadioGroupItem value="ur" id="op-ur" /><Label htmlFor="op-ur" className="font-normal">UR</Label></div>
                    <div className="flex items-center space-x-2"><RadioGroupItem value="usa" id="op-usa" /><Label htmlFor="op-usa" className="font-normal">USA</Label></div>
                </RadioGroup>
            </Field>
            ...
            <Field label="Sentido">
                <RadioGroup value={dadosOperacionais.sentido || ''} onValueChange={(v) => handleOperationalDataChange('sentido', v)} className="flex flex-row space-y-0 gap-4">
                    <div className="flex items-center space-x-2"><RadioGroupItem value="norte" id="op-norte" /><Label htmlFor="op-norte" className="font-normal">Norte</Label></div>
                    <div className="flex items-center space-x-2"><RadioGroupItem value="sul" id="op-sul" /><Label htmlFor="op-sul" className="font-normal">Sul</Label></div>
                </RadioGroup>
            </Field>
        </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8">
                    <Field label="Sexo">
                        <RadioGroup value={victim.dados_cadastrais.sexo || ''} onValueChange={v => handleVictimChange(victim.id, 'dados_cadastrais', 'sexo', v)} className="flex flex-row space-y-0 gap-4">
                            <div className="flex items-center space-x-2"><RadioGroupItem value="M" id={`sexo-m-${victim.id}`} /><Label htmlFor={`sexo-m-${victim.id}`} className="font-normal">M</Label></div>
                            <div className="flex items-center space-x-2"><RadioGroupItem value="F" id={`sexo-f-${victim.id}`} /><Label htmlFor={`sexo-f-${victim.id}`} className="font-normal">F</Label></div>
                        </RadioGroup>
                    </Field>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    <Field label="Trauma">
                        <div className="flex flex-col space-y-2">
                            <div className="flex items-center space-x-2"><Checkbox id={`trauma-acidente-${victim.id}`} checked={victim.evento?.trauma?.includes('acidente')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'trauma', 'acidente', !!c)} /><Label htmlFor={`trauma-acidente-${victim.id}`} className="font-normal">Acidente Automobilístico</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`trauma-queimadura-${victim.id}`} checked={victim.evento?.trauma?.includes('queimadura')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'trauma', 'queimadura', !!c)} /><Label htmlFor={`trauma-queimadura-${victim.id}`} className="font-normal">Queimadura</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`trauma-atropelamento-${victim.id}`} checked={victim.evento?.trauma?.includes('atropelamento')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'trauma', 'atropelamento', !!c)} /><Label htmlFor={`trauma-atropelamento-${victim.id}`} className="font-normal">Atropelamento</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`trauma-queda-${victim.id}`} checked={victim.evento?.trauma?.includes('queda')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'trauma', 'queda', !!c)} /><Label htmlFor={`trauma-queda-${victim.id}`} className="font-normal">Queda de Altura</Label></div>
                    ...
                    <Field label="Atendimento Clínico">
                         <div className="flex flex-col space-y-2">
                            <div className="flex items-center space-x-2"><Checkbox id={`clinico-mal-${victim.id}`} checked={victim.evento?.clinico?.includes('mal_subito')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'clinico', 'mal_subito', !!c)} /><Label htmlFor={`clinico-mal-${victim.id}`} className="font-normal">Mal Súbito</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`clinico-intoxicacao-${victim.id}`} checked={victim.evento?.clinico?.includes('intoxicacao')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'clinico', 'intoxicacao', !!c)} /><Label htmlFor={`clinico-intoxicacao-${victim.id}`} className="font-normal">Intoxicação Exógena</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`clinico-parto-${victim.id}`} checked={victim.evento?.clinico?.includes('parto')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'clinico', 'parto', !!c)} /><Label htmlFor={`clinico-parto-${victim.id}`} className="font-normal">Assistência ao Parto</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`clinico-convulsao-${victim.id}`} checked={victim.evento?.clinico?.includes('convulsao')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'clinico', 'convulsao', !!c)} /><Label htmlFor={`clinico-convulsao-${victim.id}`} className="font-normal">Convulsão</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`clinico-psiquiatrico-${victim.id}`} checked={victim.evento?.clinico?.includes('psiquiatrico')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'clinico', 'psiquiatrico', !!c)} /><Label htmlFor={`clinico-psiquiatrico-${victim.id}`} className="font-normal">Distúrbio Psiquiátrico</Label></div>
                     ...
                     <Field label="Condições de Segurança">
                         <div className="flex flex-col space-y-2">
                            <div className="flex items-center space-x-2"><Checkbox id={`seg-cinto-${victim.id}`} checked={victim.evento?.seguranca?.includes('cinto')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'seguranca', 'cinto', !!c)} /><Label htmlFor={`seg-cinto-${victim.id}`} className="font-normal">Cinto de Segurança</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`seg-cadeirinha-${victim.id}`} checked={victim.evento?.seguranca?.includes('cadeirinha')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'seguranca', 'cadeirinha', !!c)} /><Label htmlFor={`seg-cadeirinha-${victim.id}`} className="font-normal">Cadeirinha</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`seg-airbag-${victim.id}`} checked={victim.evento?.seguranca?.includes('airbag')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'seguranca', 'airbag', !!c)} /><Label htmlFor={`seg-airbag-${victim.id}`} className="font-normal">Air Bag</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`seg-capacete-${victim.id}`} checked={victim.evento?.seguranca?.includes('capacete')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'seguranca', 'capacete', !!c)} /><Label htmlFor={`seg-capacete-${victim.id}`} className="font-normal">Capacete</Label></div>
                    ...
                    <Field label="Cinemática">
                        <div className="flex flex-col space-y-2">
                            <div className="flex items-center space-x-2"><Checkbox id={`cin-colisao-${victim.id}`} checked={victim.evento?.cinematica?.includes('colisao')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'cinematica', 'colisao', !!c)} /><Label htmlFor={`cin-colisao-${victim.id}`} className="font-normal">Colisão</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`cin-capotamento-${victim.id}`} checked={victim.evento?.cinematica?.includes('capotamento')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'cinematica', 'capotamento', !!c)} /><Label htmlFor={`cin-capotamento-${victim.id}`} className="font-normal">Capotamento</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`cin-atropelamento-${victim.id}`} checked={victim.evento?.cinematica?.includes('atropelamento')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'cinematica', 'atropelamento', !!c)} /><Label htmlFor={`cin-atropelamento-${victim.id}`} className="font-normal">Atropelamento</Label></div>
                            <div className="flex items-center space-x-2"><Checkbox id={`cin-queda-${victim.id}`} checked={victim.evento?.cinematica?.includes('queda')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'cinematica', 'queda', !!c)} /><Label htmlFor={`cin-queda-${victim.id}`} className="font-normal">Queda</Label></div>
                             <div className="flex items-center space-x-2"><Checkbox id={`cin-tombamento-${victim.id}`} checked={victim.evento?.cinematica?.includes('tombamento')} onCheckedChange={(c) => handleVictimCheckboxChange(victim.id, 'evento', 'cinematica', 'tombamento', !!c)} /><Label htmlFor={`cin-tombamento-${victim.id}`} className="font-normal">Tombamento</Label></div>
                ...
                <div className="space-y-4">
                  <Field label="Condição Inicial">
                      <RadioGroup value={victim.avaliacao.condicao_inicial || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao', 'condicao_inicial', v)} className="flex flex-wrap gap-x-4 gap-y-2">
                        <div className="flex items-center space-x-2"><RadioGroupItem value="alerta" id={`ci-alerta-${victim.id}`} /><Label htmlFor={`ci-alerta-${victim.id}`} className="font-normal">Alerta</Label></div>
                        <div className="flex items-center space-x-2"><RadioGroupItem value="verbaliza" id={`ci-verbaliza-${victim.id}`} /><Label htmlFor={`ci-verbaliza-${victim.id}`} className="font-normal">Verbaliza</Label></div>
                        <div className="flex items-center space-x-2"><RadioGroupItem value="doloroso" id={`ci-doloroso-${victim.id}`} /><Label htmlFor={`ci-doloroso-${victim.id}`} className="font-normal">Estímulo Doloroso</Label></div>
                        <div className="flex items-center space-x-2"><RadioGroupItem value="inconsciente" id={`ci-inconsciente-${victim.id}`} /><Label htmlFor={`ci-inconsciente-${victim.id}`} className="font-normal">Inconsciente</Label></div>
                        <div className="flex items-center space-x-2"><RadioGroupItem value="deambulando" id={`ci-deambulando-${victim.id}`} /><Label htmlFor={`ci-deambulando-${victim.id}`} className="font-normal">Deambulando</Label></div>
                        <div className="flex items-center space-x-2"><RadioGroupItem value="ao_solo" id={`ci-ao_solo-${victim.id}`} /><Label htmlFor={`ci-ao_solo-${victim.id}`} className="font-normal">Ao Solo</Label></div>
                        <div className="flex items-center space-x-2"><RadioGroupItem value="ejetado" id={`ci-ejetado-${victim.id}`} /><Label htmlFor={`ci-ejetado-${victim.id}`} className="font-normal">Ejetado</Label></div>
                        <div className="flex items-center space-x-2"><RadioGroupItem value="encarcerado_retido" id={`ci-encarcerado_retido-${victim.id}`} /><Label htmlFor={`ci-encarcerado_retido-${victim.id}`} className="font-normal">Encarcerado/Retido</Label></div>
                      </RadioGroup>
                  </Field>
                </div>
                 ...
                 <div className="space-y-4">
                    <Field label="X - Hemorragia Exsanguinante">
                        <RadioGroup value={victim.avaliacao_primaria.hemorragia || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao_primaria', 'hemorragia', v)} className="flex flex-row space-y-0 gap-4">
                            <div className="flex items-center space-x-2"><RadioGroupItem value="nao" id={`hemo-nao-${victim.id}`} /><Label htmlFor={`hemo-nao-${victim.id}`} className="font-normal">Não</Label></div>
                            <div className="flex items-center space-x-2"><RadioGroupItem value="sim" id={`hemo-sim-${victim.id}`} /><Label htmlFor={`hemo-sim-${victim.id}`} className="font-normal">Sim</Label></div>
                        </RadioGroup>
                    </Field>
                     <Field label="A - Vias Aéreas">
                        <div className="flex items-center gap-4">
                            <RadioGroup value={victim.avaliacao_primaria.vias_aereas_status || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao_primaria', 'vias_aereas_status', v)} className="flex flex-row space-y-0 gap-4">
                               <div className="flex items-center space-x-2"><RadioGroupItem value="pervias" id={`vias-pervias-${victim.id}`} /><Label htmlFor={`vias-pervias-${victim.id}`} className="font-normal">Pérvias</Label></div>
                               <div className="flex items-center space-x-2"><RadioGroupItem value="obstruidas" id={`vias-obstruidas-${victim.id}`} /><Label htmlFor={`vias-obstruidas-${victim.id}`} className="font-normal">Obstruídas por:</Label></div>
                           </RadioGroup>
                     ...
                     <Field label="B - Ventilação">
                        <RadioGroup value={victim.avaliacao_primaria.ventilacao_status || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao_primaria', 'ventilacao_status', v)} className="flex flex-wrap gap-x-4 gap-y-2">
                           <div className="flex items-center space-x-2"><RadioGroupItem value="presente" id={`vent-presente-${victim.id}`} /><Label htmlFor={`vent-presente-${victim.id}`} className="font-normal">Presente</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="ausente" id={`vent-ausente-${victim.id}`} /><Label htmlFor={`vent-ausente-${victim.id}`} className="font-normal">Ausente</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="simetrica" id={`vent-simetrica-${victim.id}`} /><Label htmlFor={`vent-simetrica-${victim.id}`} className="font-normal">Simétrica</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="assimetrica" id={`vent-assimetrica-${victim.id}`} /><Label htmlFor={`vent-assimetrica-${victim.id}`} className="font-normal">Assimétrica</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="apneia" id={`vent-apneia-${victim.id}`} /><Label htmlFor={`vent-apneia-${victim.id}`} className="font-normal">Apneia</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="eupneia" id={`vent-eupneia-${victim.id}`} /><Label htmlFor={`vent-eupneia-${victim.id}`} className="font-normal">Eupneia</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="taquipneia" id={`vent-taquipneia-${victim.id}`} /><Label htmlFor={`vent-taquipneia-${victim.id}`} className="font-normal">Taquipneia</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="gasping" id={`vent-gasping-${victim.id}`} /><Label htmlFor={`vent-gasping-${victim.id}`} className="font-normal">Gasping</Label></div>
                       </RadioGroup>
                    </Field>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                       <Field label="Pulso">
                           <RadioGroup value={victim.avaliacao_primaria.pulso || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao_primaria', 'pulso', v)} className="flex flex-row space-y-0 gap-4">
                               <div className="flex items-center space-x-2"><RadioGroupItem value="presente" id={`pulso-presente-${victim.id}`} /><Label htmlFor={`pulso-presente-${victim.id}`} className="font-normal">Presente</Label></div>
                               <div className="flex items-center space-x-2"><RadioGroupItem value="cheio" id={`pulso-cheio-${victim.id}`} /><Label htmlFor={`pulso-cheio-${victim.id}`} className="font-normal">Cheio</Label></div>
                               <div className="flex items-center space-x-2"><RadioGroupItem value="filiforme" id={`pulso-filiforme-${victim.id}`} /><Label htmlFor={`pulso-filiforme-${victim.id}`} className="font-normal">Filiforme</Label></div>
                           </RadioGroup>
                       </Field>
                       <Field label="Pele">
                           <RadioGroup value={victim.avaliacao_primaria.pele || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao_primaria', 'pele', v)} className="flex flex-row space-y-0 gap-4">
                               <div className="flex items-center space-x-2"><RadioGroupItem value="normal" id={`pele-normal-${victim.id}`} /><Label htmlFor={`pele-normal-${victim.id}`} className="font-normal">Normal</Label></div>
                               <div className="flex items-center space-x-2"><RadioGroupItem value="fria" id={`pele-fria-${victim.id}`} /><Label htmlFor={`pele-fria-${victim.id}`} className="font-normal">Fria</Label></div>
                               <div className="flex items-center space-x-2"><RadioGroupItem value="sudorese" id={`pele-sudorese-${victim.id}`} /><Label htmlFor={`pele-sudorese-${victim.id}`} className="font-normal">Sudorese</Label></div>
                           </RadioGroup>
                       </Field>
                       <Field label="Perfusão">
                           <RadioGroup value={victim.avaliacao_primaria.perfusao || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao_primaria', 'perfusao', v)} className="flex flex-row space-y-0 gap-4">
                               <div className="flex items-center space-x-2"><RadioGroupItem value="<2seg" id={`perf-menor-${victim.id}`} /><Label htmlFor={`perf-menor-${victim.id}`} className="font-normal">&lt; 2seg</Label></div>
                               <div className="flex items-center space-x-2"><RadioGroupItem value=">2seg" id={`perf-maior-${victim.id}`} /><Label htmlFor={`perf-maior-${victim.id}`} className="font-normal">&gt; 2seg</Label></div>
                           </RadioGroup>
                       </Field>
                    </div>
                     <Field label="Sangramento Ativo">
                        <RadioGroup value={victim.avaliacao_primaria.sangramento || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao_primaria', 'sangramento', v)} className="flex flex-row space-y-0 gap-4">
                           <div className="flex items-center space-x-2"><RadioGroupItem value="presente" id={`sang-presente-${victim.id}`} /><Label htmlFor={`sang-presente-${victim.id}`} className="font-normal">Presente</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="ausente" id={`sang-ausente-${victim.id}`} /><Label htmlFor={`sang-ausente-${victim.id}`} className="font-normal">Ausente</Label></div>
                       </RadioGroup>
                    </Field>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                         <Field label="Pupilas">
                             <RadioGroup value={victim.avaliacao_primaria.pupilas || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao_primaria', 'pupilas', v)} className="flex flex-row space-y-0 gap-4">
                                 <div className="flex items-center space-x-2"><RadioGroupItem value="isocoricas" id={`pupilas-iso-${victim.id}`} /><Label htmlFor={`pupilas-iso-${victim.id}`} className="font-normal">Isocóricas</Label></div>
                                 <div className="flex items-center space-x-2"><RadioGroupItem value="anisocoricas" id={`pupilas-aniso-${victim.id}`} /><Label htmlFor={`pupilas-aniso-${victim.id}`} className="font-normal">Anisocóricas</Label></div>
                             </RadioGroup>
                         </Field>
                         <Field label="Fotorreagentes">
                             <RadioGroup value={victim.avaliacao_primaria.fotorreagentes || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao_primaria', 'fotorreagentes', v)} className="flex flex-row space-y-0 gap-4">
                                 <div className="flex items-center space-x-2"><RadioGroupItem value="sim" id={`foto-sim-${victim.id}`} /><Label htmlFor={`foto-sim-${victim.id}`} className="font-normal">Sim</Label></div>
                                 <div className="flex items-center space-x-2"><RadioGroupItem value="nao" id={`foto-nao-${victim.id}`} /><Label htmlFor={`foto-nao-${victim.id}`} className="font-normal">Não</Label></div>
                             </RadioGroup>
                         </Field>
                      </div>
                     <SubSectionTitle>E - Exposição</SubSectionTitle>
                      <Field>
                           <RadioGroup value={victim.avaliacao_primaria.exposicao || ''} onValueChange={(v) => handleVictimChange(victim.id, 'avaliacao_primaria', 'exposicao', v)} className="flex flex-wrap gap-x-4 gap-y-2">
                               <div className="flex items-center space-x-2"><RadioGroupItem value="sem_lesoes" id={`expo-sem-${victim.id}`} /><Label htmlFor={`expo-sem-${victim.id}`} className="font-normal">Sem Lesões Aparentes</Label></div>
                               <div className="flex items-center space-x-2"><RadioGroupItem value="hipotermia" id={`expo-hipo-${victim.id}`} /><Label htmlFor={`expo-hipo-${victim.id}`} className="font-normal">Hipotermia</Label></div>
                           </RadioGroup>
                      </Field>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                    <Field label="Abertura Ocular">
                        <RadioGroup value={victim.escala_glasgow.abertura_ocular || ''} onValueChange={v => handleVictimChange(victim.id, 'escala_glasgow', 'abertura_ocular', v)}>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="4" id={`ocular-4-${victim.id}`} /><Label htmlFor={`ocular-4-${victim.id}`} className="font-normal">04 Espontânea</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="3" id={`ocular-3-${victim.id}`} /><Label htmlFor={`ocular-3-${victim.id}`} className="font-normal">03 Estímulo Verbal</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="2" id={`ocular-2-${victim.id}`} /><Label htmlFor={`ocular-2-${victim.id}`} className="font-normal">02 Estímulo Doloroso</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="1" id={`ocular-1-${victim.id}`} /><Label htmlFor={`ocular-1-${victim.id}`} className="font-normal">01 Ausente</Label></div>
                       </RadioGroup>
                    </Field>
                    <Field label="Resposta Verbal">
                       <RadioGroup value={victim.escala_glasgow.resposta_verbal || ''} onValueChange={v => handleVictimChange(victim.id, 'escala_glasgow', 'resposta_verbal', v)}>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="5" id={`verbal-5-${victim.id}`} /><Label htmlFor={`verbal-5-${victim.id}`} className="font-normal">05 Orientado</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="4" id={`verbal-4-${victim.id}`} /><Label htmlFor={`verbal-4-${victim.id}`} className="font-normal">04 Confuso</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="3" id={`verbal-3-${victim.id}`} /><Label htmlFor={`verbal-3-${victim.id}`} className="font-normal">03 Palavras Inapropriadas</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="2" id={`verbal-2-${victim.id}`} /><Label htmlFor={`verbal-2-${victim.id}`} className="font-normal">02 Sons Incompreensíveis</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="1" id={`verbal-1-${victim.id}`} /><Label htmlFor={`verbal-1-${victim.id}`} className="font-normal">01 Ausente</Label></div>
                       </RadioGroup>
                    </Field>
                    <Field label="Resposta Motora">
                       <RadioGroup value={victim.escala_glasgow.resposta_motora || ''} onValueChange={v => handleVictimChange(victim.id, 'escala_glasgow', 'resposta_motora', v)}>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="6" id={`motora-6-${victim.id}`} /><Label htmlFor={`motora-6-${victim.id}`} className="font-normal">06 Obedece a Comandos</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="5" id={`motora-5-${victim.id}`} /><Label htmlFor={`motora-5-${victim.id}`} className="font-normal">05 Localiza a Dor</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="4" id={`motora-4-${victim.id}`} /><Label htmlFor={`motora-4-${victim.id}`} className="font-normal">04 Retira o Membro à Dor</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="3" id={`motora-3-${victim.id}`} /><Label htmlFor={`motora-3-${victim.id}`} className="font-normal">03 Decorticação (Flexão Anormal)</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="2" id={`motora-2-${victim.id}`} /><Label htmlFor={`motora-2-${victim.id}`} className="font-normal">02 Descerebração (Extensão Anormal)</Label></div>
                           <div className="flex items-center space-x-2"><RadioGroupItem value="1" id={`motora-1-${victim.id}`} /><Label htmlFor={`motora-1-${victim.id}`} className="font-normal">01 Ausente</Label></div>
                       </RadioGroup>
                    </Field>
                </div>
                  ...
                  {[
                      ...
                      { id: 'resgate_altura', label: 'Resgate em Altura' },
                        ...
                        <div key={proc.id} className="flex items-center space-x-2">
                            ...
                            <Label htmlFor={`proc-${proc.id}-${victim.id}`} className="font-normal">{proc.label}</Label>
                        </div>
                    ))}
                </div>
                  ...
                  <div>
                    {[
                        ...
                        <div key={cond.id} className="flex items-center space-x-2">
                            ...
                            <Label htmlFor={`cond-${cond.id}-${victim.id}`} className="font-normal">{cond.label}</Label>
                        </div>
                    ))}
                     <div className="flex items-center space-x-2 mt-2">
                        ...
                        <Label htmlFor={`cond-terceiros-${victim.id}`} className="font-normal">Removido por Terceiros:</Label>
                     ...
                     <div className="flex items-center space-x-2 mt-2">
                        ...
                        <Label htmlFor={`cond-hospital-${victim.id}`} className="font-normal">Removido à Unidade Hospitalar:</Label>
                   ...
                   <div>
                     ...
                     <Field label="Código">
                         <RadioGroup value={victim.conduta.codigo || ''} onValueChange={v => handleVictimChange(victim.id, 'conduta', 'codigo', v)} className="flex flex-wrap gap-x-4 gap-y-2">
                            <div className="flex items-center space-x-2"><RadioGroupItem value="vermelho" id={`cod-vermelho-${victim.id}`} /><Label htmlFor={`cod-vermelho-${victim.id}`} className="font-normal">Vermelho</Label></div>
                            <div className="flex items-center space-x-2"><RadioGroupItem value="amarelo" id={`cod-amarelo-${victim.id}`} /><Label htmlFor={`cod-amarelo-${victim.id}`} className="font-normal">Amarelo</Label></div>
                            <div className="flex items-center space-x-2"><RadioGroupItem value="verde" id={`cod-verde-${victim.id}`} /><Label htmlFor={`cod-verde-${victim.id}`} className="font-normal">Verde</Label></div>
                            <div className="flex items-center space-x-2"><RadioGroupItem value="azul" id={`cod-azul-${victim.id}`} /><Label htmlFor={`cod-azul-${victim.id}`} className="font-normal">Azul</Label></div>
                            <div className="flex items-center space-x-2"><RadioGroupItem value="preto" id={`cod-preto-${victim.id}`} /><Label htmlFor={`cod-preto-${victim.id}`} className="font-normal">Preto</Label></div>
                        </RadioGroup>
                     </Field>
                   </div>
                </div>
